...
def plot_wd_loss(loss, path, title=None):
    plt.figure()
    plt.plot(loss)
    plt.legend(['Weight decay loss'], loc='upper right')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    if title is not None:
        plt.savefig(path + '/loss' + title + '.jpg')
...
def plot_loss_zoom(n, x, train_loss, val_loss, path, title):
    plt.figure()
    plt.plot(x, train_loss)
    plt.plot(x, val_loss)
    plt.legend(['Training', 'Validation'], loc='upper right')
    plt.xlim((0,n+len(train_loss)))
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.savefig(path + '/loss' + title + '.jpg')
    plt.show()
