...
def run_AE(model, inputdata, idx_val, batch_size, optimizer, scheduler, device, num_epochs, savepath, num_patience, loss_opt=None, compute_wd_loss=None):
    # make validation set
    ...
    for epoch in range(num_epochs):
        if compute_wd_loss != 'TRUE':
            model, train_loss = train_AE(model, traindataloader, optimizer, scheduler, device, loss_opt)
        else:
            model, train_loss, wd_loss = train_AE(model, traindataloader, optimizer, scheduler, device, loss_opt, compute_wd_loss)
            wd_losses.append(wd_loss)
        ...
        if compute_wd_loss != 'TRUE':
            ...
            print('Epoch [%d / %d] training loss: %f validation loss: %f wd loss: %f' % (epoch + 1, num_epochs, train_loss, val_loss, wd_loss))
    ...
    if compute_wd_loss != 'TRUE':
        ...
        return model, train_losses, val_losses, val_loss_min, wd_losses
...
def train_AE(model, dataloader, optimizer, scheduler, device, loss_opt=None, compute_wd_loss=None):
    ...
    if compute_wd_loss != 'TRUE':
        ...
        return model, total_loss / len(dataloader.dataset), wd_loss.to('cpu').detach().numpy().item()
